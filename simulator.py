...
if __name__ == "__main__":
    ...
    # load your bot
    bot = AutoModelForCausalLM.from_pretrained(args.model_name_or_path).to(device)
        ...
        for index, context in enumerate(
            ...
            for _ in range(5):
                ...
                inputs = bot_tokenizer(
                        " ".join(
                            ([context] + dialog if len(dialog) < 3 else dialog[-3:])
                        )+bot_tokenizer.eos_token, return_tensors="pt"
                ).to(device)
                reply_ids = bot.generate(**inputs,
                                        max_length=200,
                                        pad_token_id=bot_tokenizer.eos_token_id,  
                ...
                text = bot_tokenizer.decode(reply_ids[:, inputs['input_ids'].shape[-1]:][0], skip_special_tokens=True)
                dialog.append(text)
                if not args.disable_output_dialog:
